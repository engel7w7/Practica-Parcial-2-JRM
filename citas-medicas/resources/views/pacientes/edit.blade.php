...
@section('content')
    ...
    <form action="{{ route('pacientes.update', $paciente) }}" method="POST" style="background: white; padding: 20px; border-radius: 4px;">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="nombre_completo" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" 
                   placeholder="Ej: Juan Pérez García" required value="{{ old('nombre_completo', $paciente->nombre_completo) }}">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="email" name="email" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" 
                   placeholder="Ej: juan@example.com" required value="{{ old('email', $paciente->email) }}">
            @error('email')
                <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="tel" name="telefono" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" 
                   placeholder="Ej: 3124567890" required value="{{ old('telefono', $paciente->telefono) }}">
            @error('telefono')
                <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="direccion" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" 
                   placeholder="Ej: Calle 45 #123-45" value="{{ old('direccion', $paciente->direccion) }}">
            @error('direccion')
                <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="date" name="fecha_nacimiento" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" 
                   required value="{{ old('fecha_nacimiento', $paciente->fecha_nacimiento->format('Y-m-d')) }}">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="tipo_paciente" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.95rem;" required>
                ...
                <option value="nuevo" @if(old('tipo_paciente', $paciente->tipo_paciente) == 'nuevo') selected @endif>Nuevo</option>
                <option value="recurrente" @if(old('tipo_paciente', $paciente->tipo_paciente) == 'recurrente') selected @endif>Recurrente</option>
            </select>
            @error('tipo_paciente')
                <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
